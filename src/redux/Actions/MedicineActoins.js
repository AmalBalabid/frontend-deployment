...
export const addMedicine = (user,medicine,navegate) => {
  return (dispatch) => {
    ...
    axios
      .post("https://pharmacy-backend-test-dep.herokuapp.com/medicines", medicine, {headers:{authorization:`Bearer ${token}`}})
      .then((response) => {
...
export const deleteMedicine = (id,user) => {
  return (dispatch) => {
    ...
    dispatch(Request());
    axios
      .delete(`https://pharmacy-backend-test-dep.herokuapp.com/medicines/${id}`, {headers:{authorization:`Bearer ${token}`}})
      .then((response) => {
        dispatch(deleted());
        dispatch(getPharmacy(user));
...
export const updateMedicine = (id,user, medicine) => {
  return (dispatch) => {
    ...
    axios
      .put(`https://pharmacy-backend-test-dep.herokuapp.com/medicines/${id}`, medicine, {headers:{authorization:`Bearer ${token}`}})
      .then((response) => {
        const med = response.data;
        dispatch(add(med));
        localStorage.setItem("medicine", JSON.stringify(med));
